...
def dpgan(X_train, y_train, X_test, y_test, epsilon, delta, niter, num_teachers, batch_size, no_split, lamda, seed):
    ...
    lamda = np.sqrt(2 * np.log(1.25 * (10 ^ (int(delta))))) / epsilon
    ...
    for _ in p:
        ...
        for _ in range(num_teachers):
            ...
            # Teacher 1
            X_idx = sample_X(no, batch_size)
            X_mb = X_train[X_idx, :]

            Y_mb = np.reshape(y_train[X_idx], [batch_size, 1])

            M_real = np.ones([batch_size, ])
            M_fake = np.zeros([batch_size, ])
            M_entire = np.concatenate((M_real, M_fake), 0)

            Normal_Add = np.random.normal(loc=0.0, scale=lamda, size=batch_size * 2)

            M_entire = M_entire + Normal_Add
            M_entire = (M_entire > 0.5)
            M_mb = np.reshape(M_entire.astype(float), (2 * batch_size, 1))
