...
transformed parameters {
    ...
    vector[n_groups] int_terms;
    for (j in 1:n_groups) {
        int_terms[j] = (1 / ((2.0 * pi()) ^ (beta / 2.0) * (1 + beta) ^ 1.5 * ((sigma2[j]) ^ (beta / 2))));
    }
...
model {
    ...
    if (flag_synth == 0) {
        for (i in 1:n_synth) {
            target += beta_w * ((1 / beta) * exp(normal_lpdf(y_synth[i] | lin_pred_synth[i], sqrt(sigma2[schools_synth[i]]))) ^ (beta) - int_terms[schools_synth[i]]);
        }
    }
