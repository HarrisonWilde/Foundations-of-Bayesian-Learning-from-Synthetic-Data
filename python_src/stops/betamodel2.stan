...
model {
  ...
  //Likelihood
  for (n in 1:N) {
    target += beta_w * ((1.0 / beta) * (exp(poisson_log_lpmf(y[n] | lambda[n] + sigma * epsilon[day[n]] + sigma_1 * beta_1[officer[n]] + sigma_2 * beta_2[week[n]] + sigma_3 * beta_3[month[n]] + sigma_4 * beta_4[holiday[n]]))) ^ beta - y_int_term);
  }
