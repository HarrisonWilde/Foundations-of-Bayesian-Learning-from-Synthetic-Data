...
def append_data_frames(path, prior):
    '''
    ...
    '''
    temp = None
    for fp in os.listdir('from_cluster/outputs'):
        print(fp)
        if path in fp:
            with open(f'from_cluster/outputs/{fp}/{prior}', 'rb') as f:
                if temp is None:
                    temp = pickle.load(f)
                else:
