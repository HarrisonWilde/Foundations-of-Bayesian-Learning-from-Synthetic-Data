    ...
    progress_pmap(1:total_steps, progress=p) do i
        ...
        if sampler == "CmdStan"
            ...
            @time for (name, model) in models[myid()]
                ...
                if rc == 0
                    ...
                    ll, kld = evaluate_samples(unseen_data, dgp, samples)
                    # ll, kld, wass = myid(), myid(), myid()
                    append!(evaluations, [ll, kld])
                else
                    append!(evaluations, [NaN, NaN])
                end
        ...
        elseif sampler == "AHMC"
            ...
            @time for (name, model) in models
                ...
                ll, kl = evaluate_samples(unseen_data, dgp, chains)
                append!(evaluations, [ll, kl])
        ...
        elseif sampler == "Turing"
            ...
            @time for (name, model) in models
                ...
                ll, kl = evaluate_samples(unseen_data, dgp, vcat(chains...))
                append!(evaluations, [ll, kl])
