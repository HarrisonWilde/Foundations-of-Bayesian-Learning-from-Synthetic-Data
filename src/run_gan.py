...
from gan import generate_samples
...
def run(args):
    ...
    paths = ['plots', plot_dir, 'outputs', output_dir, 'gan']
    for i in paths:
    ...
    for dgp in tqdm(dgps, position=0):
        ...
        for prior_config in tqdm(prior_configs, leave=False, position=2):
            ...
            for alpha in tqdm(args.alphas, leave=False, position=4):

                c.set_description_str('Alpha = {}'.format(alpha))

                contaminated_data, indices = generate_samples(data, alpha)
                out = [{'chain': chain, 'Alpha': noise_config[1], 'Y Tilde Value': ytildes, 'DGP': pdf_ytilde}
                ...
                for name, model in tqdm(models.items(), leave=False, position=6):
                    ...
                    fit = run_experiment(model, args.warmup, args.iters, args.chains, contaminated_data,
                                         indices, unseen_data, ytildes, 0, *prior_config, *dgp, args.cpu_count)
